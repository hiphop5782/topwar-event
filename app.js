...
function renderBoard() {
  ...
  document.querySelectorAll(".cell").forEach((cell) => {
    ...
    const heat = count ? Math.max(0.18, count / maxCount) : 0;
    cell.style.setProperty("--pick-fill-alpha", String(0.08 + heat * 0.36));
    cell.style.setProperty("--pick-image-fill-alpha", String(0.2 + heat * 0.42));
    cell.style.setProperty("--pick-border-alpha", String(0.22 + heat * 0.58));
    cell.style.setProperty("--pick-image-border-alpha", String(0.42 + heat * 0.5));
    cell.style.setProperty("--pick-badge-alpha", String(0.5 + heat * 0.45));
...
function renderWinner() {
  ...
  const latestDraw = state.drawHistory?.[0];
  const modalKey = latestDraw?.id || `${state.winnerCell}:${winners.map((user) => user.id).join(",")}`;
  if (modalKey !== activeWinnerModalKey) {
    activeWinnerModalKey = modalKey;
    openWinnerModal({
      cell: state.winnerCell,
      winners,
...
function closeWinnerModal() {
  document.querySelectorAll(".winner-modal-backdrop, .confetti-layer").forEach((node) => node.remove());
}

function openWinnerModal({ cell, winners, drawnAt }) {
  closeWinnerModal();
  launchConfetti();
  ...
  backdrop.innerHTML = `
    <section class="winner-modal" role="dialog" aria-modal="true" aria-labelledby="winnerModalTitle">
      <button type="button" class="winner-modal-close" aria-label="닫기">×</button>
      <p class="winner-modal-kicker">당첨 발표</p>
      <h2 id="winnerModalTitle">${cellLabel(cell)} 칸</h2>
      <p class="winner-modal-subtitle">
        ${
          winners.length
            ? `축하합니다, ${escapeHtml(winners.map((user) => user.name).join(", "))}!`
            : "해당 칸을 선택한 사용자가 없습니다."
        }
      </p>
      ${
        winners.length
          ? `<div class="winner-modal-names">${winners.map((user) => `<span>${escapeHtml(user.name)}</span>`).join("")}</div>`
          : ""
      }
      ${drawnAt ? `<time>${new Date(drawnAt).toLocaleString("ko-KR")}</time>` : ""}
    </section>
  `;
  ...
  backdrop.addEventListener("click", (event) => {
    if (event.target === backdrop || event.target.closest(".winner-modal-close")) close();
  });
...
function launchConfetti() {
  ...
  const colors = ["#e1192d", "#246bfe", "#f7b801", "#19a974", "#ff6b35", "#9b5de5"];
  ...
  for (let index = 0; index < 90; index += 1) {
    ...
    piece.style.setProperty("--duration", `${2.2 + Math.random() * 1.7}s`);
